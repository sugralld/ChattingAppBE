...
def funcSendMessage(room_id, sender_id, message_obj):
    try:
        ...
        return {"status":"success","code":0,"message":result.get("message"),"data":{"message_id":result.get("message_id")}}
    except Exception as e:
        return {"status":"error","code":500,"message":str(e),"data":[]}
...
def funcDeleteMessage(message_id):
    try:
        ...
        if isinstance(result, dict) and "error" in result:
            return {"status":"error","code":404,"message":result["error"],"data":[]}
        return {"status":"success","code":0,"message":result["message"],"data":{"room_id":result.get("room_id")}}
    except Exception as e:
        return {"status":"error","code":500,"message":str(e),"data":[]}