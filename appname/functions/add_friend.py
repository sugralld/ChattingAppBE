...
def searchFriendByUsername(user_id, keyword, limit=10, page=1):
    ...
    query = """
        SELECT 
            ...
            ud.updated_at,
            CASE
                WHEN fr.sender IS NOT NULL THEN 'requested'
                ELSE 'not connected'
            END AS status
        FROM user_detail ud
        ...
        LEFT JOIN friend_request fr
            ON (
                fr.sender = %s
                AND fr.receiver = ud.user_id
                AND fr.status = 'pending'
            )
        WHERE 
            ud.user_id != %s
            AND ud.username ILIKE %s
            AND uf.user_id_first IS NULL  -- belum temenan
            AND (ud.blocked_user IS NULL OR ud.blocked_user = FALSE)
...
def sendFriendRequest(sender_id, receiver_id):
    ...
    try:
        ...
        cur.execute(
            "SELECT user_id, username FROM user_detail WHERE user_id IN (%s, %s)",
